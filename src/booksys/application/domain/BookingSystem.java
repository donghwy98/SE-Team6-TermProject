...
public class BookingSystem
{
  ...
  public void addWaitingList(Time time, String name, int wn) {
	  WaitingList w = restaurant.addWaitingList(time, name, wn);
	  waitingList.addElement(w);
	  notifyObservers();
  }
  ...
  public void selectWaiting(int wn, String name) {
	  selectedWaiting = null;
	  Enumeration enums = waitingList.elements();
	  while(enums.hasMoreElements()) {
		  WaitingList w = (WaitingList) enums.nextElement();
		  if(w.getWaitingNumber() == wn) {
			  selectedWaiting = w;
		  }
	  }
  ...
  public void deleteWaiting()
  {
	...
	currentBookings.remove(selectedWaiting) ;
	restaurant.removeWaiting(selectedWaiting) ;
	selectedWaiting = null ;
	notifyObservers() ;
      }
    }
